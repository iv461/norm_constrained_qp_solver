...
//
// Experimental implementation of the sovler for sparse matrices using the SPECTRA library (similar
// to ARPACK).
#pragma once
...
// An matrix multiplication operator that implements the block-matrix multiplication required by
// algorithm, in form of the interface required by the SPECTRA library. It is templated on the
// matrix and vector operator to be able to use it matrix-free.
...
static Eigen::Vector<Scalar, Eigen::Dynamic> solve_norm_constrained_qp_sparse(
    const Eigen::SparseMatrix<Scalar>& C, const Eigen::SparseMatrix<Scalar>& b, Scalar s) {
...
///
/// References:
/// [1] "An active-set algorithm for norm constrained quadratic problems", Nikitas Rontsis, Paul J.
/// Goulart & Yuji Nakatsukasa, https://doi.org/10.1007/s10107-021-01617-2
template <typename Scalar, typename Vec, typename Mat>
static Eigen::Vector<Scalar, Eigen::Dynamic> solve_norm_constrained_qp_sparse(const Mat& C,
                                                                              const Vec& b,
                                                                              Scalar s) {
  ...
  MOp op(C, b, s);
  /// Construct the eigensolver, 1 means "compute only one eigenvalue/eigenvector", whereas 6 is
  /// some parameter for controlling the convergence of the algorithm (6 is the default)
  ...
  int nconv = eigs.compute(Spectra::SortRule::LargestReal, /*maxit=*/1000, /*tol=*/1e-10,
  ...
  if (eigs_info != Spectra::CompInfo::Successful || nconv != 1) {
    throw std::invalid_argument(fmt::format(
        "Solver failed to converge, eigs.info() was {}, nconv was {}", int(eigs_info), nconv));
  }
  /// Get largest eigenvector. The associated eigenvalue is always real (see Theorem 1. in [1] and
  ...
  /// Now check for the hard-case as defined in [1], page 6. Solving this hard case is currently not
  /// implemented.
  if (z1_star_norm < Scalar(1e-8)) {
    throw std::invalid_argument(
        fmt::format("The solver can't solve this instance (hard case occurred, where the norm of "
                    "z1-star is close to zero (it was {}).",
                    z1_star_norm));
  }
